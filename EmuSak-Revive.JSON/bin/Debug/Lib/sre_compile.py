...
assert re.MAGIC == MAGIC, "re module mismatch"
...
def _compile(code, pattern, flags):
    ...
    if (flags & re_FLAG_IGNORECASE and
            not (flags & re_FLAG_LOCALE) and
        ...
        if op in LITERAL_CODES:
            ...
            if flags & re_FLAG_IGNORECASE:
                ...
                def fixup(literal, flags=flags):
                    return re.getlower(literal, flags)
            else:
                emit(OPCODES[op])
                fixup = None
            skip = _len(code); emit(0)
            ...
            if flags & re_FLAG_LOCALE:
                av = AT_LOCALE.get(av, av)
            elif flags & re_FLAG_UNICODE:
...
def _compile_info(code, pattern, flags):
    ...
    if not (flags & re_FLAG_IGNORECASE):
...
##         print "*** CHARSET", charset
    ...
    if prefix:
        ...
        if len(prefix) == prefix_skip == len(pattern.data):
            mask = mask + re_INFO_LITERAL
    elif charset:
        mask = mask + re_INFO_CHARSET
    emit(mask)
    # pattern length
    if lo < MAXCODE:
        emit(lo)
