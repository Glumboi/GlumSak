...
#
cd "$(dirname "$0")/.."
[ ! -d "GlumSak3AV" ] && echo "GlumSak3AV not found!" && exit -1
...
coreDir="$rootDir/GlumSak3AV"
#version="$(grep -oP '<Version>\K([0-9][.][0-9][.][0-9])(?=<\/Version>)' "$coreDir/GlumSak3AV.csproj")" # Not supported on recent macos!
version="$(perl -nle'print $& while m{<Version>\K([0-9]+[.][0-9]+[.][0-9]+)(?=<\/Version>)}g' "$coreDir/GlumSak3AV.csproj")"
