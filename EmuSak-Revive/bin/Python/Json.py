...
def GetGameIdAndIcon():
    with open('gameIcons_Ids.txt', 'w') as the_file:
        for i in range(len(nsuIds)):
            the_file.write(json.dumps(data[nsuIds[i]]["iconUrl"], indent=4)+" | " + json.dumps(data[nsuIds[i]]["id"], indent=4)+"\n")
            print(json.dumps(data[nsuIds[i]]["iconUrl"], indent=4)+" | " + json.dumps(data[nsuIds[i]]["id"], indent=4))
